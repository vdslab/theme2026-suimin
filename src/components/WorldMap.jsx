...
export default function WorldMap({ selectedCoffee, onSelectCoffee, searchQuery, drankCoffees = {}, onUpdateDrank, onRemoveDrank, recommendedCoffee }) {
  ...
  const handleCountryClick = (e, geoName) => {
    ...
    if (nodes && nodes.length > 0) {
      ...
      const topNode = [...nodes].sort((a, b) => b.sampleCount - a.sampleCount)[0];
  ...
  return (
    <div ref={containerRef} className="w-full h-full relative">
      <svg ref={svgRef} viewBox={`0 0 ${width} ${height}`} width="100%" height="100%" className="absolute inset-0 select-none bg-[#e0f2fe]" onClick={() => { onSelectCoffee(null); setPopupInfo(null); }}>
        <defs>
          {Object.entries(filteredNodesByGeoName).map(([geoName, nodes]) => {
            const totalSamples = nodes.reduce((sum, n) => sum + n.sampleCount, 0);
            let currentX = 0;
            const patternWidth = 24; // 縞模様の太さ

            return (
              <pattern key={geoName} id={`pattern-${geoName.replace(/[^a-zA-Z0-9]/g, '-')}`} width={patternWidth} height={patternWidth} patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
                {nodes.map(node => {
                   const ratio = node.sampleCount / totalSamples;
                   const stripeWidth = ratio * patternWidth;
                   
                   const isHighlighted = (selectedCoffee?.id === node.id) || (recommendedCoffee?.id === node.id);
                   const isAnyHighlighted = !!(selectedCoffee || recommendedCoffee);
                   const isFilteredOut = activeCluster !== null && activeCluster !== node.clusterName;
                   
                   let opacity = 1;
                   if (isFilteredOut) opacity = 0.15;
                   else if (isAnyHighlighted && !isHighlighted) opacity = 0.25;

                   const color = node.blendedColor || clusterColor(node.clusterName);
                   
                   const rect = <rect key={node.id} x={currentX} width={stripeWidth} height={patternWidth} fill={color} opacity={opacity} />;
        ...
        <g ref={gRef} className="countries">
          {geoFeatures.map((geo, i) => {
            ...
            const fill = hasData ? `url(#pattern-${geoName.replace(/[^a-zA-Z0-9]/g, '-')})` : "#cbd5e1";
